...
{
	const n = process.env.CI ? 1000 : 100;
	const cache = new LRU({
		max: 5,
		maxAge: n * 2,
	});
...
{
	let disposed = false as any as number;
...
{
	const l = new LRU({ length: true as any });
	expect(typeof l.lengthCalculator).toBe('function')
	l.lengthCalculator = 'not a function' as any
	expect(typeof l.lengthCalculator).toBe('function')
	done()
})
...
{
	const l = new LRU<number, number>({ max: 3 });
	l.set(2, 2)
	l.set(1, 1)
